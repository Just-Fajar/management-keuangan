      ...
      <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-white">Pengaturan Budget Per Kategori</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-3">
          {expenseCategories.map((cat) => {
            ...
            return (
              <div
                key={cat.id}
                className="p-3 bg-slate-800/60 border border-slate-700/50 rounded-2xl text-xs space-y-2"
              >
                <div className="flex items-center justify-between">
                  <span className="font-bold text-white">{cat.name}</span>
                  <span className="text-[10px] text-slate-400">
                    Saat ini: <span className="text-indigo-400 font-semibold">{formatIDR(cat.monthly_budget || 0)}</span>
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-900 border border-slate-700 text-white rounded-xl px-3 py-2 focus:outline-none focus:ring-1 focus:ring-indigo-500 font-semibold"
                  />
                  <button
                    type="button"
                    disabled={isSaving}
                    onClick={() => handleSave(cat)}
                    className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-500 active:scale-95 text-white font-bold rounded-xl transition-all flex items-center gap-1 shadow-md"
                  >
